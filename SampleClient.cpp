...
TEST(MattanTests, randomTest) {
	//Meant for aquarium computers since on other computers you might not get killed at all or with other signals.
	//If you pass but the process finishes before 100 iterations you should check to see if you can make your code more efficient
	//Also, make sure that -O2 optimization is activated by adding the flag '-02' to "target_compile_options" in your cmakelists file
	EXPECT_EXIT(randbody(), ::testing::KilledBySignal(24), ::testing::MatchesRegex(""));
}
